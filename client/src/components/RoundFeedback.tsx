...
export function RoundFeedback({
  ...
  const cardExampleKeys: Record<string, string> = allCards.reduce(
    (acc, card) => {
  ...
  if (delta.visibility_insight >= thresholds.highVisibilityDelta) {
    const relevantCard = investedCards.find((id) =>
      ['digitalTwin', 'warehouseFlow', 'gridSensors', 'emissionsDetection'].includes(id),
    );
  ...
  if (delta.efficiency_throughput >= thresholds.highEfficiencyDelta) {
    const relevantCard = investedCards.find((id) =>
      ['smartTools', 'warehouseFlow', 'fleetOptimization'].includes(id),
    );
    const example = relevantCard
      ? t(cardExampleKeys[relevantCard])
      : t('feedback.defaultAutomation');
    feedbackItems.push({
      icon: <TrendingUp className="w-5 h-5" data-testid="icon-trending-up" />,
  ...
  if (delta.sustainability_emissions >= thresholds.highSustainabilityDelta) {
    const relevantCard = investedCards.find((id) =>
      ['energyMonitoring', 'smartBuilding', 'fleetOptimization', 'emissionsDetection'].includes(id),
    );
  ...
  if (delta.early_warning_prevention >= thresholds.highEarlyWarningDelta) {
    const relevantCard = investedCards.find((id) =>
      ['predictiveMaintenance', 'healthMonitoring', 'gridSensors', 'waterMonitoring'].includes(id),
    );
    const example = relevantCard
      ? t(cardExampleKeys[relevantCard])
      : t('feedback.defaultPredictive');
    feedbackItems.push({
      icon: <CheckCircle className="w-5 h-5" data-testid="icon-check-circle" />,
  ...
  if (
    metricsAfter.complexity_risk > thresholds.criticalComplexityAbsolute &&
    delta.early_warning_prevention < thresholds.lowEarlyWarningDelta
  ) {
    feedbackItems.push({
  ...
  const allPositive = [
    delta.visibility_insight,
    delta.efficiency_throughput,
    delta.sustainability_emissions,
    delta.early_warning_prevention,
  ].every((d) => d > thresholds.balancedGrowthMinDelta);
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-5xl mx-auto space-y-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">{t('roundFeedback.subtitle')}</p>
        </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3
                  ...
                  data-testid="text-disaster-title"
                >
                ...
                {disasters.map((disaster, index) => (
                  <div key={index} className="space-y-2">
                    <p
                      ...
                      data-testid={`text-disaster-${index}`}
                    >
                      {t(disaster.copyKey)}
                    </p>
                    <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                      {Object.entries(disaster.penalties).map(
                        ([metric, penalty]) =>
                          penalty !== undefined && (
                            <span key={metric} className="px-2 py-1 bg-destructive/10 rounded">
                              {t(`common.metrics.${metric}`)}: {penalty > 0 ? '+' : ''}
                              {penalty}
                            </span>
                          ),
                      )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="p-6">
            ...
            <MetricsPanel metrics={metricsAfter} previousMetrics={metricsBefore} animate={true} />
          </Card>
        </div>
            ...
            <div className="space-y-3">
              {feedbackItems.map((item, index) => (
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed flex-1 text-foreground">{item.message}</p>
                </div>
              ))}
            </div>
          </Card>
